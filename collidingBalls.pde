...
int nbs = 2; // number of balls = nbs*nbs*nbs
float br = 20; // ball radius def 20
int Frate=20;
...
//   fill(magenta); noStroke(); show(pick(mouseX,mouseY),10);
   ...
   if(animating){
     if(CEvents.isEmpty() || frameCount%5==0){
       //Ball Ball Collision
  ...
  //ShowBallNumbers();
  scribe("First Collision Time:"+FirstCollisionTime,300,40);
...
void keyPressed() {
  ...
  if(key=='i') {P.initPointsOnGrid(nbs,w,br,cyan); stop=false; CEvents.clear(); animating = true; }
  ...
  if(key=='.') P.insertClosestProjection(Of); // Inserts new vertex in P that is the closeset projection of O
...
String title ="6491-2015-P1: Animation of colliding balls", name ="Bhuvanesh Shanmuga Sundaram",
